class VideoSaver {
    ...
    // --- ФУНКЦИИ ДЛЯ ОПРЕДЕЛЕНИЯ ТИПА И ИЗВЛЕЧЕНИЯ ID ---

    getVideoType(url) {
        if (!url) return 'other';
        // Учитываем короткие ссылки vt.tiktok.com
        if (url.includes('tiktok.com') || url.includes('vt.tiktok.com')) return 'tiktok';
        // Учитываем youtube.com/shorts/
        if (url.includes('youtube.com/shorts/') || url.includes('youtu.be/shorts/') || url.includes('youtube.com/watch')) return 'youtube';
        if (url.includes('vk.com/video')) return 'vk';
        return 'other';
    }

    // Улучшенное извлечение ID для VK (работает с разными форматами)
    extractVKVideoId(url) {
        const patterns = [
            /video(-?\d+)_(\d+)/, // стандартная ссылка videoXXXX_YYYY
            /clip(-?\d+)_(\d+)/,  // для клипов
            /oid=(-?\d+)&id=(\d+)/ // из iframe кода
        ];
    ...
    // Извлечение ID для YouTube (работает с youtube.com/shorts/XXXXX)
    getYouTubeId(url) {
        const patterns = [
            /youtube\.com\/shorts\/([a-zA-Z0-9_-]{11})/, // для shorts
            /youtu\.be\/([a-zA-Z0-9_-]{11})/,           // для youtu.be
            /youtube\.com\/watch\?v=([a-zA-Z0-9_-]{11})/ // для обычных ссылок
        ];
        for (let pattern of patterns) {
            const match = url.match(pattern);
    ...
    // Извлечение ID для TikTok (из коротких ссылок)
    getTikTokId(url) {
        // Короткие ссылки vt.tiktok.com не содержат ID в явном виде.
        // Лучший способ - вставить саму ссылку в iframe TikTok.
        // Возвращаем оригинальный URL для вставки.
        return url;
    }
    ...
    // --- УНИВЕРСАЛЬНАЯ ФУНКЦИЯ ПРОИГРЫВАНИЯ ---
    ...
    playVideo(video) {
        ...
        // --- ЛОГИКА ДЛЯ РАЗНЫХ ТИПОВ ВИДЕО ---

        if (video.type === 'vk') {
            const videoIds = this.extractVKVideoId(video.url);
            if (videoIds) {
                // Формируем iframe точно так, как вы предложили
                iframeSrc = `https://vk.com/video_ext.php?oid=${videoIds.ownerId}&id=${videoIds.videoId}&hash=9e787c195a300793`;
                // Примечание: hash может быть уникальным для каждого видео.
                // Если видео не работает, возможно, hash нужно получать отдельно или использовать параметр &autoplay=1
                // Для универсальности можно использовать базовый URL без hash, но это может не работать для всех видео.
                // Лучше использовать URL, который дает сам VK.
                // Попробуем вариант без hash, но с autoplay:
                iframeSrc = `https://vk.com/video_ext.php?oid=${videoIds.ownerId}&id=${videoIds.videoId}&autoplay=1`;
            }
            ...
            // TikTok требует вставки ссылки на видео в специальном формате
            // Обычно используется oEmbed, но проще всего дать ссылку на сам TikTok.
            // Попробуем embed вариант (может работать не для всех видео из-за ограничений)
            // Вместо этого дадим пользователю кнопку для открытия в новой вкладке.
            playerContainer.innerHTML = `
                <div style="text-align: center; padding: 50px; background: #f0f0f0; color: #333;">
                    <p>Для просмотра этого TikTok видео, пожалуйста, откройте его в новой вкладке.</p>
                    <a href="${this.escapeHtml(video.url)}" target="_blank" rel="noopener noreferrer" style="display: inline-block; padding: 10px 20px; background-color: #000000; color: white; text-decoration: none; border-radius: 5px; margin-top: 10px;">
        ...
        // Если удалось сформировать src для iframe
        if (iframeSrc) {
            ...
            // Если тип не определён или не поддерживается (и это не TikTok, для которого мы уже вывели ссылку)
            playerContainer.innerHTML = `
                <div style="text-align: center; padding: 50px; background: #f0f0f0; color: #333;">
                    <p>Не удалось загрузить видео. Попробуйте открыть его напрямую.</p>
                    <a href="${this.escapeHtml(video.url)}" target="_blank" rel="noopener noreferrer" style="color: #4A76A8;">Открыть ссылку</a>
                </div>
            `;
        }

        // Обновляем информацию о видео в плеере
